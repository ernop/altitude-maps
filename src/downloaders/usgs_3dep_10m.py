...
def download_usgs_3dep_10m_tiles(
    ...
    """
    Download 10m USGS 3DEP data as 1-degree tiles and merge.
    
    This is the main entry point for 10m data downloads. It follows the unified
    1-degree tiling architecture used by 30m and 90m data for maximum reuse.
    ...
    for i, tile_bounds in enumerate(tiles, 1):
        ...
        if tile_path.exists():
            ...
            print(f"  [{i}/{len(tiles)}] Using cached tile: {tile_filename} ({file_size_mb:.2f} MB)")
            tile_paths.append(tile_path)
            continue
        
        # Download the tile
        print(f"  [{i}/{len(tiles)}] Downloading: {tile_filename}")
        success = download_single_tile_10m(tile_bounds, tile_path, dataset)
